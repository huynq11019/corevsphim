...
$(document).ready(function() {
    ...
    function buildCommentHtml(comment) {
        var avatar = comment.user.avatar || '/themes/thempho/images/default.jpg';
        var timeAgo = 'vừa xong'; // Hoặc có thể format từ comment.created_at
        ...
        <div class="flex gap-3 lg:gap-4 bg-primary-comment px-2 py-2 lg:px-4 rounded-lg lg:py-2">
            <div class="shrink-0 w-8 lg:w-10">
                <div class=" relative w-full aspect-square">
                    <span style="box-sizing: border-box; display: block; overflow: hidden; width: initial; height: initial; background: none; opacity: 1; border: 0px; margin: 0px; padding: 0px; position: absolute; inset: 0px;">
                        <img title="${comment.user.name}" alt="${comment.user.name}" src="${avatar}" class="rounded-full" style="position: absolute; inset: 0px; box-sizing: border-box; padding: 0px; border: none; margin: auto; display: block; width: 0px; height: 0px; min-width: 100%; max-width: 100%; min-height: 100%; max-height: 100%; object-fit: cover;">
                    </span>
                </div>
            </div>
            <div class="grow">
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.status === 'success') {
                    ...
                    // Thêm comment mới vào đầu danh sách thay vì reload trang
                    if (response.comment) {
                        var newCommentHtml = buildCommentHtml(response.comment);
                        $('.flex.flex-col.gap-2.mt-2').prepend(newCommentHtml);
                    }
